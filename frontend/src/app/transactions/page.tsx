        ...
        <div className="grid gap-8 xl:grid-cols-[1.2fr_0.8fr]">
          ...
          <section className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-[0_18px_50px_rgba(15,23,42,0.06)] flex flex-col">
            <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 pb-5">
              <div>
                ...
                <h1 className="mt-2 text-4xl font-semibold text-[#0B2545]">Transaction history</h1>
                <p className="mt-3 text-sm leading-6 text-slate-600">
                  Review posted, pending, and scheduled payments. Click on any record to inspect the captured **Saathi AI Telemetry Guard Profile**.
                </p>
              </div>
            </div>
            ...
            <div className="mt-6 flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="relative max-w-xs w-full">
                <input
                  className="w-full rounded-2xl border border-slate-200 px-4 py-2 outline-none transition focus:border-slate-400 text-xs pl-8"
                    ...
                    filteredRows.map((row) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-4 font-semibold text-slate-900">{row.beneficiary}</td>
                          <td className={`px-4 py-4 font-bold ${row.amount.startsWith('+') ? 'text-emerald-600' : 'text-slate-900'}`}>
                            {row.amount}
                          </td>
                          <td className="px-4 py-4">
                            <span className={`rounded-full px-3 py-1 text-[9px] font-bold uppercase tracking-wider border ${
          ...
          <aside className="space-y-6">
            {activeAudit ? (
                ...
                <div>
                  <div className="flex items-center justify-between border-b border-white/10 pb-4">
                    <div>
                      ...
                      <h2 className="text-lg font-bold text-white mt-0.5">Session telemetry profile</h2>
                    </div>
                    <span className="text-[10px] font-mono text-[#F37021]">{selectedTxn}</span>
                  </div>
                  ...
                  <div className="mt-5 flex items-center gap-4 bg-white/5 border border-white/5 rounded-2xl p-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-extrabold mt-0.5 tracking-tight flex items-center gap-1.5">
                        {activeAudit.threatLevel === 'HIGH' ? '🚨 SCAM SUSPECTED (BLOCKED)' : '🛡️ SESSION SECURE (PASSED)'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mt-5 space-y-2.5 text-xs border-t border-white/5 pt-4">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">{activeAudit.ip}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">{activeAudit.latency}</span>
                    </div>
                    <div className="flex justify-between">
              ...
              <div className="rounded-[2rem] border border-dashed border-slate-300 p-8 text-center text-slate-500 text-xs">
