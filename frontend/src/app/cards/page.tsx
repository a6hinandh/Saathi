...
export default function CardsPage() {
  ...
  const handlePinSubmit = () => {
    if (enteredPin.length === 4) {
      setPinSuccess(`✔️ PIN for Card ending in ${pinCard?.slice(-4)} has been successfully updated.`);
      setEnteredPin('');
      setTimeout(() => {
        setPinCard(null);
        setPinSuccess('');
  ...
  const handleTravelSubmit = () => {
    setTravelSuccess(`✈️ International Travel Notice activated for ${travelCountry} on Card ending in ${travelCard?.slice(-4)}.`);
    setTimeout(() => {
      setTravelCard(null);
      setTravelSuccess('');
    }, 3000);
      ...
      <main className="mx-auto max-w-7xl px-6 py-8 lg:px-8">
                ...
                >
                  ...
                  {isFrozen && (
                    <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-xs z-10 flex flex-col items-center justify-center text-center p-6 transition-all duration-300">
                      ...
                      <button
                        ...
                        className="mt-4 rounded-full bg-red-600 hover:bg-red-700 transition px-5 py-2 text-xs font-bold uppercase tracking-wider text-white"
                      >
                        Unfreeze Card
                      </button>
                    </div>
                  ...
                  <div>
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <span className="text-[10px] uppercase font-bold tracking-[0.25em] text-[#F37021]">
                          {isRuPay ? 'Saathi Secure Net' : 'Saathi Select'}
                        </span>
                        <h2 className="mt-1 text-2xl font-bold tracking-tight text-white">{card.name}</h2>
                        <p className="mt-1 text-sm font-mono text-white/50 tracking-widest">{card.number}</p>
                      </div>
                      <span className="rounded-full bg-white/10 border border-white/20 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-amber-400">
                        {card.status}
                      </span>
                    </div>
                    ...
                    <div className="mt-8 flex items-center justify-between gap-4 border-t border-white/5 pt-4 text-xs text-white/70">
                      <div>
                        <p className="text-[9px] uppercase tracking-widest text-white/40">Cardholder</p>
                        <p className="font-semibold text-white mt-0.5">ASHA MENON</p>
                      </div>
                      <div className="text-right">
                        <p className="text-[9px] uppercase tracking-widest text-white/40">Active limit</p>
                  ...
                  <div className="mt-8 grid grid-cols-2 gap-3 text-xs">
                    <button
                      onClick={() => toggleFreeze(card.number)}
                      className="rounded-2xl border border-white/10 bg-white/5 hover:bg-white/10 transition px-4 py-3 text-left font-semibold flex items-center justify-between"
                    >
                      <span>🔒 Freeze Card</span>
                      <span className="text-[9px] text-[#F37021] font-bold">Lock</span>
                    </button>
                    ...
                    >
                      ...
                      <span className="text-[9px] text-emerald-400 font-bold">Adjust</span>
                    </button>
                    ...
                    >
                      ...
                      <span className="text-[9px] text-blue-400 font-bold">Reset</span>
                    </button>
                    ...
                    <button
                      ...
                      className="rounded-2xl border border-white/10 bg-white/5 hover:bg-white/10 transition px-4 py-3 text-left font-semibold flex items-center justify-between"
                    >
                      <span>✈️ Global Notice</span>
                      <span className="text-[9px] text-amber-400 font-bold">Travel</span>
                    </button>
                  </div>
                </article>
              );
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 px-6 backdrop-blur-sm animate-fadeIn">
            <div className="max-w-sm w-full rounded-[2rem] bg-slate-900 border border-slate-800 p-6 text-white shadow-2xl">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <h3 className="text-md font-bold tracking-wide">🔒 Secure PIN Update</h3>
                <button onClick={() => setPinCard(null)} className="text-slate-400 hover:text-white text-lg">×</button>
              </div>

              {pinSuccess ? (
                <div className="my-6 p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold text-center leading-5">
                  {pinSuccess}
                </div>
              ) : (
                <div className="mt-4 space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-2 py-2">
                    ...
                    <button
                      ...
                      className="col-span-2 bg-[#F37021] hover:bg-[#E05A17] disabled:opacity-40 transition rounded-xl py-3 text-xs font-bold uppercase tracking-wider"
                    >
                      Confirm PIN
                    </button>
                  </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 px-6 backdrop-blur-sm animate-fadeIn">
            <div className="max-w-md w-full rounded-[2rem] bg-white border border-slate-100 p-6 text-slate-900 shadow-2xl">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <h3 className="text-md font-bold tracking-wide text-[#0B2545]">✈️ Add Travel Notice</h3>
                <button onClick={() => setTravelCard(null)} className="text-slate-400 hover:text-slate-950 text-xl font-bold">×</button>
              </div>

              {travelSuccess ? (
                <div className="my-6 p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-semibold text-center leading-5">
                  {travelSuccess}
                </div>
              ) : (
                <div className="mt-4 space-y-4">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <select
                      className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs outline-none transition focus:border-slate-400"
                      value={travelCountry}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="flex flex-col gap-1">
                      ...
                      <input type="date" className="rounded-xl border border-slate-200 px-3 py-2 text-xs outline-none text-slate-700" defaultValue="2026-05-25" />
                    </div>
                    <div className="flex flex-col gap-1">
                      <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">End Date</label>
                      <input type="date" className="rounded-xl border border-slate-200 px-3 py-2 text-xs outline-none text-slate-700" defaultValue="2026-06-10" />
                    </div>
                  </div>
        ...
        {limitCard && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 px-6 backdrop-blur-sm animate-fadeIn">
            <div className="max-w-md w-full rounded-[2rem] bg-white border border-slate-100 p-6 text-slate-900 shadow-2xl">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <h3 className="text-md font-bold tracking-wide text-[#0B2545]">📈 Set Retail Limit</h3>
