...
export default function SecurityPage() {
  ...
  const handlePanicSubmit = (e: React.FormEvent) => {
    ...
    if (tempPin.length === 4 && /^\d+$/.test(tempPin)) {
      ...
      setToastMessage('Silent Duress Panic PIN successfully registered! Duress Mode active.');
        ...
        <div className="grid gap-8 xl:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="mt-8 border-t border-white/10 pt-6">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <h4 className="text-sm font-bold text-white flex items-center gap-2">
                    ...
                    <span className={`rounded-full px-2 py-0.5 text-[8px] font-bold uppercase tracking-wider border ${
                      panicPin ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-amber-500/10 text-amber-400 border-amber-500/20'
                    }`}>
                      {panicPin ? 'Active Mode' : 'Not Configured'}
                    </span>
                  </h4>
                  <p className="text-xs text-white/50 mt-1 max-w-sm leading-4 font-medium">
                ...
                <button
                  ...
                  className="rounded-full bg-amber-400 hover:bg-amber-500 transition px-5 py-2 text-xs font-extrabold text-slate-900 tracking-wide uppercase"
                >
          ...
          <section className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-[0_18px_50px_rgba(15,23,42,0.06)] flex flex-col justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-2 font-medium">Toggle specific SDK data streams sent to the real-time AI Risk Fusion Engine.</p>
            ...
            <div className="mt-8 border-t border-slate-100 pt-5 text-center text-[10px] text-slate-500 italic font-semibold flex items-center justify-center gap-1">
              <svg className="w-3.5 h-3.5 text-emerald-500 shrink-0 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
              </svg>
              <span>Session telemetry is securely encrypted at the local browser level. Raw entries are never logged.</span>
            </div>
          </section>
        </div>
              ...
              <form onSubmit={handlePanicSubmit} className="mt-4 space-y-4">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    maxLength={4}
                    pattern="\d{4}"
                    className="rounded-xl border border-slate-200 px-3 py-2.5 text-center tracking-widest text-lg font-bold outline-none transition focus:border-slate-400 text-slate-800"
                    placeholder="••••"
                    value={tempPin}
